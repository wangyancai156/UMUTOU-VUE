...
namespace WangYc.Repository.NHibernate.Repositories
{
    public abstract class Repository<T, TEntityKey> where T : IAggregateRoot
    {
        ...
        public ListPaged<T> PagedFindBy(Query query, int index, int count)
        {
            ICriteria criteriaQuery = getcriteriaQuery(query);
            int totalCount = (int)criteriaQuery.SetProjection(Projections.RowCount()).List()[0];
            int pageCount = totalCount / count + (totalCount % count > 0 ? 1 : 0);
            int pageIndex = Math.Min(pageCount, index);
            ICriteria criteriaQuerya = getcriteriaQuery(query);
            return new ListPaged<T>
            {
                EntityList = criteriaQuerya.SetMaxResults(count).SetFirstResult((index - 1) * count).List<T>(),
        ...
        private ICriteria getcriteriaQuery(Query query)
        {
